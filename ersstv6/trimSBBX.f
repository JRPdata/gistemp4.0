...
C****
      if(iargc()==0) then
        write(6,*) "Usage: trimSBBX.exe in_fname [ out_fname"
        stop
      end if
      call getarg(1,title)
      open(10,file=trim(title),form='unformatted')
      if(iargc()>1) then
        call getarg(2,title)
        open(11,file=trim(title),form='unformatted')
      else
        open(11,file=trim(title)//'.trim',form='unformatted')
      end if
